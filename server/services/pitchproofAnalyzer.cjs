...
function assertSafePublicUrl(rawUrl) {
  ...
  if (isUnsafeHost(parsed.hostname)) {
    throw new Error(`Blocked private or internal URL: ${parsed.hostname}`);
  }
  return parsed;
}
...
function fetchPage(rawUrl, timeout = 7000, redirects = 0) {
  return new Promise((resolve, reject) => {
    let urlObj;
    try { urlObj = assertSafePublicUrl(rawUrl); }
    ...
    const req = lib.get({
      ...
      if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
        ...
        let next;
        try {
          next = new URL(loc, urlObj).toString();
        ...
        res.resume();
        return fetchPage(next, timeout, redirects + 1).then(resolve).catch(reject);
      }
...
function requestLink(url, method, timeoutMs, redirects = []) {
  return new Promise(resolve => {
    let urlObj;
    try {
      urlObj = assertSafePublicUrl(url);
    } catch {
...
async function crawlWithPlaywright(startUrl) {
  ...
  try {
    ...
    await context.route('**/*', route => {
      try {
        assertSafePublicUrl(route.request().url());
      } catch {
        return route.abort();
      }
